...
> {
  ...
  return withWrapper((wrappedRequestHeaders) => client.request<${
    o.operationResultType
      ...
      const operationName = this._operationsToInclude.length === 1 ? `${this._operationsToInclude[0].node.name.value}${capitalizeFirstLetter(this._operationsToInclude[0].node.operation)}` : 'get';
    return `${additionalExportedTypes}

const defaultWrapper: SdkFunctionWrapper = (action, _operationName, _operationType) => action();
${extraVariables.join('\n')}
export function ${operationName}Sdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
${allPossibleActions.join(',\n')}
  };
}
export type Sdk = ReturnType<typeof ${operationName}Sdk>;`;
  }
}
